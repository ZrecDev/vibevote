...
`VIBEVOTE_RATE_LIMIT_KEY_SECRET` is required, server-only, and must be a distinct 32+-character secret; `VIBEVOTE_RATE_LIMIT_TIMEOUT_MS` is optional and nonsecret, must be an integer from 100 to 10000 milliseconds, and defaults to 1000. Vercel system environment variables must be exposed so `VERCEL=1`, `VERCEL_ENV`, and `VERCEL_PROJECT_ID` are available at runtime. Missing or malformed deployment/provider configuration, a provider timeout/network error, or a malformed provider response maps to the existing safe 503 response; a saturated policy maps to safe 429. There is no in-memory or permissive deployed fallback. Development and test intentionally return allowed without contacting a provider so the isolated local stack remains usable.

Before enabling a preview or production deployment, apply the migrations in timestamp order to its Supabase project, then configure `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `VIBEVOTE_APP_ORIGIN`, and a distinct `VIBEVOTE_RATE_LIMIT_KEY_SECRET` for that Vercel environment; expose Vercel system variables and set `VIBEVOTE_RATE_LIMIT_TIMEOUT_MS` only when the default is unsuitable. `VIBEVOTE_APP_ORIGIN` must exactly match the public origin used by that deployment, including its preview alias. Verify the final migration is present before enabling traffic, then verify create, join, and bootstrap against the deployed API: saturation must return 429, while an unavailable provider must return 503 without exposing provider details. Rotate the rate-limit HMAC secret through Vercel environment configuration and redeploy; rotation intentionally starts fresh rate-limit windows, so retain the previous value only for rollback until old deployments are retired. Rollback requires reverting the application deployment only after retaining the database RPC and table needed by any still-running deployment. During a provider incident, retain the fail-closed 503 behavior, investigate the server-only provider configuration and Supabase availability, and do not introduce an in-memory bypass.
