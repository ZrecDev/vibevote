...
declare
  ...
  v_expired record;
begin
  select * into v_first from public.check_session_rate_limit_v1('production', repeat('a', 64), 2, 1);
  select * into v_second from public.check_session_rate_limit_v1('production', repeat('a', 64), 2, 1);
  select * into v_denied from public.check_session_rate_limit_v1('production', repeat('a', 64), 2, 1);
  select * into v_preview from public.check_session_rate_limit_v1('preview', repeat('a', 64), 2, 1);
  ...
  select * into v_expired from public.check_session_rate_limit_v1('production', repeat('a', 64), 2, 1);
