-- Separate rate-limit windows for Vercel projects that share a Supabase database.
-- Existing rows lack project identity and expire within 24 hours, so discard them before
-- enforcing the new namespace format rather than attributing them to the wrong project.
delete from private.session_rate_limit_windows;

alter table private.session_rate_limit_windows
  drop constraint session_rate_limit_windows_namespace_check,
  add constraint session_rate_limit_windows_namespace_check
    check (namespace ~ '^(preview|production):prj_[A-Za-z0-9]+$');
...
begin
  if p_namespace !~ '^(preview|production):prj_[A-Za-z0-9]+$'
  ...
  insert into private.session_rate_limit_windows as current_window (
    namespace, key_hash, window_started_at, request_count
