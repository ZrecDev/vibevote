-- Private voting foundation. Ballots are never returned by public room functions.
create table public.private_ballots (
  ...
  value text not null check (value in ('LOVE','FINE','PASS','VETO')),
  submitted_at timestamptz not null default now(),
  primary key (participant_id, option_id)
);
create index private_ballots_session_participant_idx on public.private_ballots(session_id, participant_id);
...
create table public.session_results (
  ...
  winner_option_id uuid not null references public.decision_options(id),
  method text not null check (method in ('INSTANT_MATCH','BEST_FIT','CHAOS')),
  explanation text not null check (char_length(btrim(explanation)) between 1 and 500),
  finalized_at timestamptz not null default now()
);

alter table public.private_ballots enable row level security;
alter table public.session_results enable row level security;
revoke all on public.private_ballots, public.session_results from anon, authenticated;
...
revoke all on function public.submit_private_ballot_v1(uuid,text,jsonb), public.finalize_decision_v1(uuid,text) from public, anon, authenticated;
grant execute on function public.submit_private_ballot_v1(uuid,text,jsonb), public.finalize_decision_v1(uuid,text) to service_role;
